<template>
  <div class="search-container">
    ...
    <search-history
      v-else
      :search-histories="searchHistroies"
    />
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      searchHistroies: getItem('search-histories') || [] // 搜索历史记录
    }
  },
  computed: {
    ...mapState(['user'])
  ...
  methods: {
    onSearch (searchText) {
      ...
      // 把最新的搜索记录放到顶部
      this.searchHistroies.unshift(searchText)

      // 如果没有登录,则把搜索记录保存到本地
      setItem('search-histories', this.searchHistroies)
